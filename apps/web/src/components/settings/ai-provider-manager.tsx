...
interface AiProviderManagerProps {
  /** Which provider set to read and mutate — sent explicitly on every call. */
...
/**
 * The AI provider CRUD surface, shared by the tenant console (/settings) and
 * the global platform console (/admin/ai-providers). The only difference
 * between the two is the `scope` it operates on and its copy, so the UI lives
 * here once rather than being duplicated per screen.
 */
export function AiProviderManager({ scope, title, description }: AiProviderManagerProps) {
  const { data: configs, isLoading } = useAiProviderConfigs(scope);
  const createMutation = useCreateAiProvider(scope);
  const updateMutation = useUpdateAiProvider(scope);
  const deleteMutation = useDeleteAiProvider(scope);
          ...
          {isLoading ? (
            ...
            <div className="rounded-md border border-dashed p-6 text-center">
              <p className="text-sm text-muted-foreground mb-3">
                No AI providers configured yet. Add a provider to enable AI form
                generation.
              </p>
              {availableProviders.length > 0 && (
                ...
                >
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      ...
                      <span>
                        Key: <span className="font-mono">{config.apiKeyMasked}</span>
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
          ...
          <div className="space-y-4 py-2">
            ...
            {editConfig &&
              (editConfig.provider === 'ollama' ||
                editConfig.provider === 'minimax' ||
                editConfig.provider === 'kimi') && (
                <div className="space-y-2">
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong>{deleteConfirm?.displayName}</strong>? This action cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>

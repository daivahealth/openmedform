...
- Platform extensions ride ONLY under options.omf, e.g. { "options": { "omf": { "control": "textarea", "screen": { "colSpan": 6, "rows": 4, "inline": true }, "print": { "minHeightMm": 30, "border": true } } } }.
  ...
  Additional omf keys (use where the source shows them): "accentColor" (a hex colour approximating a coloured section border/header, e.g. "#c0392b" for a red CARDIOVASCULAR box), "icon" (a single emoji matching the section's pictogram, e.g. "❤️", "🎂", "🫁", "🔬", "🧠", "🦠", "🔪", "🦴", "🛏️", "🩸"), "points" (the numeric point value printed on a scored checkbox row), "optionPoints" (points per enum CODE for a scored single-select, e.g. { "NO": 0, "YES": 25 }), "optionLabels" (display text per enum CODE, when the dataSchema uses a plain "enum" rather than "oneOf" titles), "pointLegend" (array of the distinct point values shown as chips in a section header, e.g. [1,2,3]), "hideSectionTotal" (true on a Group to suppress the automatic live "Σ n" subtotal chip its header gets when it contains scored fields — scoring itself is unaffected; use when the user asks to remove that badge from a section), "showSectionTotal" (true on a Group that CONTAINS scoring sections and whose own combined total the source prints — the automatic chip is drawn on the innermost scoring box only, so an outer box stays quiet unless you set this; do not set it on a box that merely groups scored sections without printing a total), "bands" (score → verdict table, see SCORED THRESHOLDS below).

...
- CONDITIONAL VISIBILITY: a uiSchema element may carry a "rule" so it appears only when another field has a given value. Use it for a field the source shows only on a specific choice — the classic "Please specify…" box beside an "Other" option:
  ...
  A rule may also sit on a LAYOUT element, and that is what you want when the source reveals a whole section rather than one box — including an "OmfTableRow", where the rule belongs on the ROW and never on the Controls inside it, so the row appears as a unit. Use this for a stepwise assessment whose later steps are only asked when an earlier answer requires them (CAM-ICU: assess Feature 2 only when Feature 1 is present), and keep every step's fields — a gated step is a real step, not an optional one.
- MULTI-FIELD CONDITIONS: a condition "scope" of "#" matches the WHOLE response object, so its "schema" can combine several fields with ordinary JSON Schema — "properties" + "required" for AND, "anyOf" for OR. Use it for a COMPUTED OUTCOME the form states in words ("POSITIVE only if Feature 1 is present AND Feature 2 is present AND (Feature 3 is present OR Feature 4 is present)"): emit one "Label" per outcome, each carrying the condition for its own case.
  ...
  List in "required" every field the case depends on, so a form that has not been filled in yet shows no outcome at all rather than a false one. Outcomes must be mutually exclusive: exactly one may match any given response. This is for a rule the source states in PROSE; a numeric total mapped to risk bands is "scoreSummary" with omf.bands, not this.
  Give each outcome Label an omf.accentColor so it reads as the banner the source draws rather than as body text. Colour it by CLINICAL SENSE, not by decoration: an abnormal / positive / "action required" result red ("#b3392c"), a normal / negative / reassuring one green ("#2e7d4f"), an intermediate or cautionary one amber ("#b8860b"). Note that a colour the source expresses through a STYLESHEET class rather than an inline style cannot be read (stylesheets are stripped), so judge from what the outcome MEANS.
- Platform extensions ride ONLY under options.omf, e.g. { "options": { "omf": { "control": "textarea", "screen": { "colSpan": 6, "rows": 4, "inline": true }, "print": { "minHeightMm": 30, "border": true } } } }.
  omf.control values you may use: "textarea", "radio", "checkboxGroup", "scoringMatrix", "vitalSignsChart", "colorCodedGrid", "riskStratification", "signatureDate", "clinicalReferenceTable", "checklistMatrix", "scoreSummary", "recordTable".
  Additional omf keys (use where the source shows them): "accentColor" (a hex colour approximating a coloured section border/header, e.g. "#c0392b" for a red CARDIOVASCULAR box; on a "Label" it instead renders that text as a bordered, tinted, bold CALLOUT — use it for a banner/box the source draws around a result, an alert or a warning, and leave it off ordinary instruction text), "icon" (a single emoji matching the section's pictogram, e.g. "❤️", "🎂", "🫁", "🔬", "🧠", "🦠", "🔪", "🦴", "🛏️", "🩸"), "points" (the numeric point value printed on a scored checkbox row), "optionPoints" (points per enum CODE for a scored single-select, e.g. { "NO": 0, "YES": 25 }), "optionLabels" (display text per enum CODE, when the dataSchema uses a plain "enum" rather than "oneOf" titles), "pointLegend" (array of the distinct point values shown as chips in a section header, e.g. [1,2,3]), "hideSectionTotal" (true on a Group to suppress the automatic live "Σ n" subtotal chip its header gets when it contains scored fields — scoring itself is unaffected; use when the user asks to remove that badge from a section), "showSectionTotal" (true on a Group that CONTAINS scoring sections and whose own combined total the source prints — the automatic chip is drawn on the innermost scoring box only, so an outer box stays quiet unless you set this; do not set it on a box that merely groups scored sections without printing a total), "bands" (score → verdict table, see SCORED THRESHOLDS below).
- SCORED THRESHOLDS ("Positive if ≥ 2"): when a scored block prints its own verdict beside its total, put "bands" on that block's Group. Each band is { "minScore"?, "maxScore"?, "label", "color"? }, bounds inclusive, and the section's OWN subtotal picks the matching one — the renderers draw it next to the "Σ n" chip.
    { "type": "Group", "label": "qSOFA (1 pt each)",
      "options": { "omf": { "bands": [ { "maxScore": 1, "label": "Negative", "color": "#2e7d4f" },
                                       { "minScore": 2, "label": "Positive", "color": "#b3392c" } ] } },
      "elements": [ <one boolean Control per criterion, each with omf.points> ] }
  Put the bands on the Group of the instrument they belong to. A sheet may carry SEVERAL independent instruments (qSOFA out of 3, SIRS out of 4) — each gets its own Group and its own bands, and you must NOT total them together. Use "scoreSummary" (which stratifies the WHOLE form) only when the source really does print one combined score for the entire sheet.
  If a threshold applies to a value the user TYPES rather than to a total of tick-boxes ("MEWS: positive if score > 3", entered as one number), there is no section total to band: emit the number Control and a rule-gated "Label" per verdict, using the root-scope condition described above ({ "scope": "#", "schema": { "type": "object", "properties": { "mewsScore": { "minimum": 4 } }, "required": ["mewsScore"] } }).
...
- LAYOUT FIDELITY (the renderer boxes and spaces these automatically — use them):
  ...
  - MULTI-SELECT (choose ANY of a short inline option row, e.g. "Contact ☐ Droplet ☐ Airborne ☐ Neutropenic ☐" or "HBV ☐ HCV ☐"): emit ONE Control with omf.control "checkboxGroup", bound to an ARRAY property whose "items" carries the coded options and titles: { "type": "array", "uniqueItems": true, "items": { "type": "string", "oneOf": [ { "const": "HBV", "title": "HBV" }, … ] } }. The stored value is the array of selected codes. Use this ONLY for a compact same-line option row; a vertical checklist of full-sentence items stays one boolean Control per row (those often carry per-row points or qualifiers), and it is NEVER "checklistMatrix" (that is a rows × columns grid and needs omf.rows/columns).
...
- ARRAYS: only use "type":"array" for a repeating list the user can ADD TO and REMOVE FROM (an open-ended "add another medication" table — see recordTable below), or for a "checkboxGroup" multi-select of coded options (see MULTI-SELECT above). Any other fixed set of tick-box options is NOT an array: emit one boolean Control per option, or an enum when the options are mutually exclusive. An array bound to no visible options renders as an empty "Add to …" widget that does not exist on the source form.

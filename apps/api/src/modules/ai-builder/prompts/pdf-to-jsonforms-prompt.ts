...
- Platform extensions ride ONLY under options.omf, e.g. { "options": { "omf": { "control": "textarea", "screen": { "colSpan": 6, "rows": 4, "inline": true }, "print": { "minHeightMm": 30, "border": true } } } }.
  ...
  Additional omf keys (use where the source shows them): "accentColor" (a hex colour approximating a coloured section border/header, e.g. "#c0392b" for a red CARDIOVASCULAR box), "icon" (a single emoji matching the section's pictogram, e.g. "❤️", "🎂", "🫁", "🔬", "🧠", "🦠", "🔪", "🦴", "🛏️", "🩸"), "points" (the numeric point value printed on a scored checkbox row), "pointLegend" (array of the distinct point values shown as chips in a section header, e.g. [1,2,3]).
- SCORED CLINICAL CHECKLISTS / COLOUR-CODED DOMAIN BOXES (critical — do NOT drop the rows):
  ...
  - When a single-select group of options each carries points (e.g. AGE bands 41–60 / 61–74 / ≥75 where only one applies), still emit each band as its own scored boolean Control unless the source draws mutually-exclusive radio circles, in which case use an enum + omf.control "radio".

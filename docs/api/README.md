...
| PATCH | /api/forms/:id/field-meta | Set/clear a field's or a **section's** previous-values setting and a field's unit ([ADR-006](../ADR/006-section-level-history.md)): `{ target: { scope } \| { pointer }, history?: { show, count?, trend? } \| null, unit?: string \| null }`. `pointer` addresses a layout element (a Group) by its `/elements/N` path in `uiSchema.layout`; `unit` is refused on a section. Draft edited in place, published forks — audited as `form.field-meta.update` |

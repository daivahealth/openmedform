...
| GET | /api/admin/usage | Token spend grouped along one dimension: `?groupBy=user\|form\|tenant\|provider\|operation` (default `user`), optionally windowed with `?from=`/`?to=` (ISO dates; 400 on unparseable). Returns platform `totals` plus `rows` of `{ key, label, calls, inputTokens, outputTokens, totalTokens, lastUsedAt }` sorted by tokens desc. The `operation` dimension answers the AI-cost questions (issue #128): which pipeline dominates, and how big is a typical call — its rows additionally carry `outputP50`/`outputP95` (output tokens per call, computed over the windowed rows). Rows with no key surface as `Unattributed` so the grouped rows always reconcile with the total; a key whose entity was deleted labels as `(deleted)`. |

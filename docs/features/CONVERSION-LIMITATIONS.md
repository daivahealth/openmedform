...
- **Size caps** (160 fields, 160 table rows, 32k chars, 2 MB) exist because one
  ...
  `CONVERSION_MAX_TOKENS`, `CONVERSION_MAX_SOURCE_CHARS` — defaults 160 / 160 /
  40 960 / 32 000): an operator whose providers can emit more (OpenAI
  ...
  truncate — such a deployment lowers `CONVERSION_MAX_TOKENS` instead. A
  repeating log counts as the single `recordTable` it becomes rather than as its
  rendered rows, so a chart the page fills with blank rows at load is not
  rejected for rows the model never has to emit.

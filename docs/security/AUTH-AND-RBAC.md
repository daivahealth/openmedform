...
| Rule | Exception |
|---|---|
| **Scripts are stripped** and never shown to the model. | Two reads, neither of them execution. (1) With a per-upload opt-in, they are **parsed** (acorn, AST only — no `eval`, no `Function`, no VM) for named *literal* config: option lists, thresholds, reference tables. Any value containing an identifier, call, template hole, spread or function is discarded whole. Hard caps on bytes parsed, entries, depth, string length and total size. (2) Always, they are scanned for the **target** of a visibility toggle or a text write (`getElementById('x')` … `x.style.display =` / `x.textContent =`, including an element handed straight to a helper that writes through its parameter) so a progressively-revealed section or a computed result banner can be identified — the target only, never the logic, and no script string ever reaches the model. The `<script>` element is removed from the cleaned HTML either way. |
